...
0. **記録依頼は DOCUMENT であり、本節の調査開始ではない**  
   次を含む依頼はフェーズを DOCUMENT に固定する。単語「調査」だけでは新規調査にしない。

   - `調査として` かつ (`docs/調査` または `MD` または `いつものフォーマット`)
   - `調査MDにして` / `docs/調査` へのMD出力 / Failure Knowledgeを記録

   DOCUMENTでは本節の検索義務（次項）を適用しない。セッション内の既知証拠を即ファイル化する。フォーマット確認以外の追加tool、新仮説の深掘り、精度目的の再計測は、続けて調べる明示がない限り禁止。不足は本文の`未確認`とし、埋めに戻らない。先に出力を揃え、評価はファイルの後にする。

0b. **結論後の探索世代を開かない**  
   「enough」「判断材料が揃った」「評価が出そろった」「書くだけが残」「ほぼ完了」など、依頼の結論を一度述べたあとは、同じ依頼の探索tool（grep／sed／git log／cat 等）を続けない。終端は最終回答、または宣言済みの write／apply_patch／検証だけとする。同一探索コマンドの反復も禁止。`codex-switch rencrow` の loop-brake hook がこの境界を機械的に拒否する。

0c. **最新のユーザー指示は常に最優先**  
   以前の方針・結論・推奨案・別作業・進行中の復旧より、最新のユーザー指示を常に優先する。最新指示の完了条件だけを進め、無関係な調査・復旧・探索へ逸れない。`codex-switch rencrow` の loop-brake hook は state に最新指示を保持し、結論後探索などの拒否理由に最新指示へ戻す文言を付ける。**System／Tools 領域への additionalContext／動的プロンプト積み増しは禁止**（KV prefix を壊す）。遠隔ホストやdaemon／configの調査は `/tmp` の細切れスクリプト連打を避け、必要な確認を1本にまとめる（超過分は hook が拒否する）。

0d. **System／Tools 領域は固定 prefix のみ**  
   System と Tools（schema／description）の内側へ、turn中・tool呼び出しごとにプロンプトや guidance を積まない。許可されるのはセッション開始時に決まる固定 prefix。動的な指示・拒否理由・観測結果は user／assistant／tool result（suffix）側だけに置く。Tools Schema の正本 revision は `RenCrow_LLM/artifacts/tools-schema-fixed-20260917.md`（Serena `fixed_tools` ＋ `~/.codex/config.toml` MCP）。一覧変更は revision 更新を伴う明示変更とし、セッション途中の MCP 追加を禁止する。

1. **仕様なし報告を禁止する**（DOCUMENT中は適用しない）  
