...
- RenCrow系の構築・運用について過去の操作を知る必要がある場合は、ユーザーへ記憶を尋ねる前、または情報が存在しないと判断する前に、`~/.codex` と `~/.rencrow` の両rootにあるsessions・操作ログ、runtime logs・receipts、旧履歴を読み取り専用で確認する。
- 期間、module、Run・Task・job・trace IDなどで対象を絞り、CLIで現在のsource・state・active configと照合し、既存の調査・receiptを再利用する。履歴の参照path・行・時刻と、確定／不明を記録する。
- 履歴は証拠であり、現行仕様やactive configの正本を上書きしない。文字列の出現、宛先、LLM報告を、実Actorまたは実行成功の証拠と混同しない。
- アクセス不可・欠落・照合不能はその境界を明示し、secretや会話全文を出力しない。旧コマンドの再実行や外部操作は、この確認だけでは許可されない。
